...
- execution width and whether parameters/noise were generated by the canonical
  batched path or injected into a resolved single-sound path;
...
The batch dimension is not a hardware requirement. Canonical synth1B1 corpus
identity uses TorchSynth's supported reproducible batches, while the device
consumes one resolved patch and selected noise stream. Batch-1 versus batched
floating evaluation is a named portability experiment; any drift is reported
at the first differing trace rather than attributed to the hardware.
