...
the record states: while DR-0008's status is Proposed, no choice is accepted,
and consumers must refuse not-yet-accepted values wherever the contract
requires accepted ones.

Status vocabulary follows DR-0008 Section 12 (``proposed`` / ``selected`` /
``accepted`` / ``rejected``); every entry shipped today carries the operator
ruling annotation plus the pending-ratification qualifier:
``selected (operator ruling 2026-09-19); pending ratification``.

:func:`require_accepted` is the structural refusal gate: it raises while a
choice is not accepted, so candidate instantiations cannot silently be
...
def validate_choices(payload: Dict[str, Any]) -> None:
    ...
    for choice in choices:
        cid = choice.get("id")
        if not cid:
            raise ValueError("every choice must carry an id")
        if cid in seen:
            raise ValueError(f"duplicate choice id: {cid}")
        seen.add(cid)
        status = choice.get("status", "")
        if "accepted" in status and not status.startswith("selected"):
...
def accepted_choices(payload: Dict[str, Any] = None) -> Dict[str, Dict[str, Any]]:
    """Return only choices whose status is ``accepted`` (empty while Proposed)."""
    payload = payload if payload is not None else load_choices()
    return {
        choice["id"]: choice
        for choice in payload["choices"]
...
def require_accepted(choice_id: str, payload: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    Raises :class:`ChoiceNotAccepted` while DR-0008 is Proposed, per the
    consumer rule in DR-0008 Section 12 and issue #53 ("Choices are data").
    """
    payload = payload if payload is not None else load_choices()
