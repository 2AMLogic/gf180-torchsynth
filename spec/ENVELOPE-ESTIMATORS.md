...
`--check` executes the grid, checks each expected control outcome, rederives
stored rows from their diagnostic bytes, and compares saved observations to
the replay within `1e-9` absolute/relative serialization precision. It checks
implementation/preregistration hashes and writes nothing. This replay bound is
not a hardware tolerance or cross-platform byte-identity assertion.
